...
# Change sart dates for thrre young foxes to start week 35
# Per, Ludde, Jens
names(df_foxcol)
df_foxcol[,date.begin:=fifelse(FoxID=="Ludde", as.IDate("2016-08-27"), date.begin)]
df_foxcol[,date.begin:=fifelse(FoxID=="Per", as.IDate("2014-08-27"), date.begin)]
df_foxcol[,date.begin:=fifelse(FoxID=="Jens", as.IDate("2018-08-27"), date.begin)]
## Edit wrong date
df_foxcol[,date.begin:=fifelse(FoxID=="Molly", as.IDate("2013-01-20"), date.begin)]
df_foxcol[FoxID=="Molly"]
names(df_foxcol)
## Collar file 
...
# merged[, S_MONTH := month(date.begin)]
# merged[, S_DAY := mday(date.begin)]
...
with(merged,table(E_YEAR))
with(merged,table(E_WEEK))
...
fox_surv <- data.table(id=rep('id', 2785), sw=rep(0, 2785), ew=rep(0,2785), cens=rep(9, 2785), sex=rep('sex', 2785),age=rep('age', 2785), site=rep('site', 2785),cause=rep(999,2785), SY=rep(999,2785), EY=rep(999,2785)) 
str(fox_surv)
