...
## Remove Alfa
df_foxcol <- df_foxcol[FoxID!="Alfa"]
df_foxcol[, S_Y := S_YEAR - 2010]
df_foxcol[, E_Y := E_YEAR - 2010]
df_foxcol[Age=="SA" & S_WEEK<W & S_WEEK>27 ]
...
merged3 <- df_foxcol2[,-c(3,6) ] 
merged3[, CENS := fifelse(CAUSE<7,1,0)]
with(merged3,table(CENS))
## 43 dödstillfällen
with (merged3,table(CAUSE, Sex, Age))
with (merged3[CAUSE<7],table(CAUSE, Sex))
...
# Which foxes were captured as subadults and survived more than week 36 the next year!
                ...
                fox_surv[rad,5] <- merged3$Sex[i]
                fox_surv[rad,6] <- merged3$Age[i]
...
head(fox_surv)
merged3[Sex=="F"  & CENS == 1 & Weeks<3, FoxID]
merged3[Sex=="M" &  CENS == 1 & Weeks<3, FoxID]

remove <- c("Rocio", "Stephanie", "Zea", "Anders",   "Atna", "Drevsjo2", "Kniven",   "Kvilten" )
fox_surv2 <- fox_surv[!(id %in% remove),]
head(fox_surv2)


a1<-survfit(Surv(sw2,ew2,cens) ~ 1 , data = fox_surv2)
plot(a1)
a2<-survfit(Surv(sw2,ew2,cens) ~ sex , data = fox_surv2)
plot(a2)
a3<-survfit(Surv(sw2,ew2,cens) ~ age , data = fox_surv2)
plot(a3)
a4<-survfit(Surv(sw2,ew2,cens) ~ age + sex , data = fox_surv2)
plot(a4)

###########################################  Kolla unga honor tidigt #############
# Kolla sample size subadult-sex
m1<-coxph(Surv(sw2,ew2,cens)~1, data=fox_surv2)
m2<-coxph(Surv(sw2,ew2,cens)~sex, data=fox_surv2)
m3<-coxph(Surv(sw2,ew2,cens)~age, data=fox_surv2)
m4<-coxph(Surv(sw2,ew2,cens)~age+sex, data=fox_surv2)
m5<-coxph(Surv(sw2,ew2,cens)~age*sex, data=fox_surv2)
...
# AdFem 0.677   0.141   0.450       1
# AdMal 0.534   0.0984  0.373   0.767
# SAFem 0.538   0.152   0.310   0.937
# SAMal 0.373   0.109   0.211   0.660

SA_M_1 <- fox_surv[age=="SA" & sex == "M"]  # 658
dim(SA_M_1)
AD_M_1 <- fox_surv[age=="AD" & sex == "M"]  # 1009
dim(AD_M_1)

SA_F_1 <- fox_surv[age=="SA" & sex == "F"]  # 416
dim(SA_F_1)
